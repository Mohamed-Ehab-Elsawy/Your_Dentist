            ...
            Column(
                ...
                LazyColumn(
                    ...
                    items(dentists.size) { index ->
                        DentistItem(dentists[index], onBookNowClick = { dentist ->
                            AppProviders.dentist = dentist
                            navController.navigate(PatientScreens.BookAppointment.route)
                        })
                    }
                }
            }
    ...
    when (uiState) {
        ...
        is UiState.Success -> {
            LaunchedEffect(uiState) {
                ...
                dentists = if (!newList.isEmpty())
                    (uiState as UiState.Success<List<Dentist>>).data
                else
                    emptyList()
            }
        }
